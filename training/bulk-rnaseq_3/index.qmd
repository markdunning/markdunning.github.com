...
Whereas a gene-concept network plot (`cnetplot`) depicts the linkages of genes and biological concepts (e.g. GO terms or KEGG pathways) as a network. This plot can identify the core "hub" genes (genes with high connectivity) that are common to multiple enriched pathways. These hub genes are often considered the most critical players driving the observed biological change.
...
The Top Panel: Enrichment Score (ES) & $\text{GSEA}$ Barcode. This panel shows the core evidence for enrichment. + Running $\text{ES}$ Curve (The Line): This is a running sum statistic that measures the degree of enrichment. It starts at zero and increases every time a gene belonging to the pathway is encountered in the ranked list, whereas it slightly decreases when a gene not in the pathway is encountered. The peak of this curve is the maximum Enrichment Score ($\text{ES}$). If the peak is on the left (positive), the pathway is up-regulated; if the peak is on the right (negative), the pathway is down-regulated. + $\text{GSEA}$ Barcode (The Ticks): The black vertical lines beneath the curve are the barcode. Each line represents a single gene from the pathway, placed at its exact rank position. The clustering of these ticks—either on the far left or far right—provides the visual evidence for the calculated $\text{ES}$.

-   The Bottom Panel: Gene Ranking Metric displays the value of the statistic used to rank all genes.
    -   Ranking Metric: This usually shows the $\text{Log}_2 \text{Fold Change}$ ($\text{LFC}$), $t$-statistic, or another metric for every gene in the experiment.
    -   Interpretation: This confirms that the genes on the far left of the plot have the most positive values (high $\text{LFC}$) and those on the far right have the most negative values (low $\text{LFC}$), providing context for the directional enrichment observed in the top panel.The plot therefore provides a concise, integrated view, confirming both statistical significance (from the $\text{NES}$ not shown on the curve but derived from it) and the directionality and magnitude of the pathway's regulation.
...
The special feature of a KEGG pathway is its standardized, structured graphical representation as a set of computable maps. This makes that visualisation with tools like `pathview` is possible. The standardization means KEGG is more than just a list of genes — it's an integrated system where both the visual elements and the underlying molecular data are tightly linked.


In the below we take a pathway such as "Lysosome", use it's standard KEGG layout and colour the genes involved in the pathway according to the LFC.
...
### Pathways analysis for non-human organisms
