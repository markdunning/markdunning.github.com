...
```{r}
plotCounts(dds,"ENSG00000119681",intgroup = "condition")
```

```{r}
plotCounts(dds,"ENSG00000148175",intgroup = "condition")
```
...
- Re-run the analysis to find differentially-expressed genes between the `IR` treated samples and `TGF`
...
```{r eval=FALSE}
select(org.Hs.eg.db, keys="ENSG00000119681",
...
```{r}
AnnotationDbi::select(org.Hs.eg.db, keys="ENSG00000119681",keytype = "ENSEMBL",columns=c("SYMBOL","GENENAME"))
```
