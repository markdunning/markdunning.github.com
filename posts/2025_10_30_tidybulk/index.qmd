...
I have been analysing RNA-seq data for rather longer than I care to mention, and have mostly stuck to the same tried and tested workflow. Whilst re-writing [my tutorials](https://mdbioinformatics.com/training.html) for these pages it occurred to me that the materials, that mostly use base R, would benefit from some of the same tidy methodology I employ on a daily basis for general data manipulation and visualisation. Fortunately in recent years there has been a drive to introduce tidy frameworks for omics. Can this new framework replace my daily workhorse?
...
```{r doDESeq, warning=TRUE,message=TRUE}
counts_de <- caf_tidy %>% 
...
The package also looks to have [batch-correction](https://www.bioconductor.org/packages/devel/bioc/vignettes/tidybulk/inst/doc/introduction.html#35_Step_4:_Batch_Effect_Correction) built-in via the ComBatSeq method, which sounds incredibly convenient.

I've also noticed there are methods for [de-convolution and correcting for the celluarity of samples](https://www.bioconductor.org/packages/devel/bioc/vignettes/tidybulk/inst/doc/introduction.html#37_Step_6:_Cellularity_Analysis). I don't have any particular need for this right now (at least, I don't think I do), but good to know that it is easy to integrate.
