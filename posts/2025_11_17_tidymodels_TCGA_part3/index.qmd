...
theme: darkly
image: preview.png
description: "Machine Learning with glmnet and random forests in R using many features (genes)"
---
...
In the first section in this series we described how to download TCGA data for breast cancers and manipulated them using a combination of `tidybulk` and `dplyr` to retain a set of expressed, variable genes plus a set of known cancer genes.
...
The genes with negative "importance" are in fact only very slightly so, and there only a handful of them.
