...
theme: darkly
image: preview.png
description: "Identifying predictive markers for ER status from high-dimensional gene expression data. We compare the performance of penalized regression (GLMNet) and ensemble learning (Random Forests) for feature selection and classification"
---
...
In [Part 1 of this series](https://mdbioinformatics.com/posts/2025_11_06_tidymodels_TCGA_part1/) we described how to download TCGA data for breast cancers and manipulated them using a combination of `tidybulk` and `dplyr` to retain a set of expressed, variable genes. There are a set of packages that we will need for this Part:-
...
You also need the processed data from Part 1 and the code to download this is:-
...
[Part 2](https://mdbioinformatics.com/posts/2025_11_06_tidymodels_TCGA_part2/) was hopefully a gentle introduction to Machine Learning, and we didn't set the bar too high for our task. The simple models we built using `ESR1` to classify breast cancer patients into Estrogen Receptor `Positive` or `Negative` were extremely effective (as measured by accuracy, sensitivity and specificity). Faced with this task in reality there wouldn't be much justification in exploring other models when simple models, that are easily explainable, perform very well.
...
Even though we have told the model what value of $\lambda$ we want to use, it is trying out different values of $\lambda$ and assessing the model fit at each point. In practical terms, this would allow us to tune the behaviour of the model but that is beyond the scope of this particular section and shouldn't be performed directly on the training data. By printing the coefficients of the model we can see what genes it has "shrunken" to zero and effectively discarded from the model.
...
There is clearly an opportunity for some improvement, and that could be achieved by better choice of *hyper-parameters*. For now though lets see how other methods perform starting with the decision tree.
...
Extracting this information can be done as follows. We see that **TFF1** is the most important which connects with the decision tree we built above.
...
The genes with negative "importance" are in fact only very slightly so, and there only a handful of them. Moreover, we can combine these importance metrics with the Lasso output from above to compare how the two methods are considering genes as important. Reassuring, those with negative importance in the random forest, with the exception of **GSTM1** all had a coefficient of 0 (i.e. were removed by Lasso).
